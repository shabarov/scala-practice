...
object Main extends App {
  ...
  //ex02
  def f_(x : Int) = Future[String] { "" + x + "." + x }
  def g_(y : String) = Future[Double] { y.toDouble }
  ...
  //ex03
  def f3(x : String) = Future { x + "one! " }
  def f4(x : String) = Future { x + "two! " }
  def f5(x : String) = Future { x + "three!" }
